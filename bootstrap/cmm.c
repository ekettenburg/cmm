...
#endif
/* cmm — a self-contained native compiler for the cmm language.
 *
...
/* ===================================================================== */
static void *xmalloc(size_t n){ void *p=malloc(n?n:1); if(!p){fprintf(stderr,"cmm: out of memory\n");exit(1);} return p; }
static void *xrealloc(void *q,size_t n){ void *p=realloc(q,n?n:1); if(!p){fprintf(stderr,"cmm: out of memory\n");exit(1);} return p; }
...
static const char *NATIVE_NS[] = {"Console","Math","File","Date","Json","Socket","Http","Sys","Lambda","Zip","Crypto","Base64","Mysql","Preg","Serve"};
...
static void init_tables(void){
    ...
    static NSMethod nsm[] = {
        ...
        {"Crypto","hmacSha1","cmm_crypto_hmac_sha1",&T_STRING,2},
        {"Crypto","hotp","cmm_crypto_hotp",&T_STRING,3},
        {"Crypto","timingSafeEqual","cmm_crypto_timing_safe_equal",&T_BOOL,2},
        ...
        {"Mysql","connectTls","cmm_mysql_connect_tls",&T_INT,6},
        ...
        {"Http","parseRequest","cmm_req_parse",&T_DATA,1},
        {"Serve","listen","cmm_serve_listen",&T_BOOL,1},
        {"Serve","next","cmm_serve_next",&T_STRING,0},
        {"Serve","respond","cmm_serve_respond",&T_BOOL,1},
...
static char *codegen_generate(const char *entry){
    ...
    emit(&out,"/* Generated by cmm. Do not edit. */");
...
/* ---- import resolution ---- */
static char *g_self_dir=NULL;    /* set in main(); dir of the cmm executable */
...
/* Directory containing the running cmm executable (for bundled toolchains). */
...
/* Resolved C toolchain (always zig cc, unless CMMC_CC/--cc overrides). */
...
static void setup_target(void){
    ...
    const char *zt; int stat=0; const char *tos="linux";
    ...
    else if(!strcmp(g_target_name,"windows-x64")||!strcmp(g_target_name,"windows")||
            !strcmp(g_target_name,"win64")){ zt="x86_64-windows-gnu"; tos="windows"; }
    else if(!strcmp(g_target_name,"windows-arm64")){ zt="aarch64-windows-gnu"; tos="windows"; }
    else { die("unknown --target '%s' (use: al2023, al2023-arm64, al2023-static, windows-x64, windows-arm64)",g_target_name); return; }
    g_target_os=tos; if(stat) g_static=1;
    if(!g_cc && !g_cc_override && !getenv("CMMC_CC")){
        char *zig=find_zig();
        if(!zig) die("cross-compiling needs zig (one toolchain for all hosts).\n"
                     "  install once:  pip install ziglang     (Windows and Linux)\n"
                     "  or put zig from ziglang.org on PATH, or set CMMC_ZIG=<path>.");
        g_cc=sfmt("%s cc -target %s",zig,zt);
    }
}
...
/* One toolchain: zig. An explicit CMMC_CC / --cc override (assumed gcc-like) is
   honored for power users; otherwise zig cc is required for every target. */
static void resolve_toolchain(void){
    if(g_cc) return;
    const char *ov = g_cc_override ? g_cc_override : getenv("CMMC_CC");
    if(ov && *ov){ g_cc=xstrdup(ov); return; }
    char *zig=find_zig();
    if(zig){ g_cc=sfmt("%s cc",zig); return; }
    /* g_cc stays NULL -> compile_to_exe() emits the install-zig error */
}
...
static char *ensure_tls_lib(int verbose){
    ...
    mkdir_p(cdir);
    char *obj=path_join(cdir, target_is_windows()?"cmmtls_v2.a":"cmmtls_v2.o");
    ...
    if(target_is_windows()){
        ...
        char *arc=path_join(cdir,"cmmtls_v2.a");
...
/* Build (once) and cache the compiled runtime object for this target + flags.
   The ~4000-line runtime otherwise recompiles on every build and dominates the
   time; caching it makes warm rebuilds several times faster. Keyed by target,
   flags, and a hash of the runtime source (+ TLS config), so a changed runtime
   or config invalidates the cache automatically. Returns the .o path or NULL. */
static char *ensure_runtime_obj(const char *runtime_c, const char *tmpdir,
                                int use_tls, int no_verify, int dbg, int verbose){
    unsigned long long h=1469598103934665603ULL;   /* FNV-1a over runtime src */
    char *src=read_file(runtime_c);
    if(src){ for(char *p=src;*p;p++){ h^=(unsigned char)*p; h*=1099511628211ULL; } free(src); }
    if(use_tls && g_tls_dir){                        /* config affects layout */
        char *cfg=read_file(sfmt("%s/cmm_mbedtls_config.h",g_tls_dir));
        if(cfg){ for(char *p=cfg;*p;p++){ h^=(unsigned char)*p; h*=1099511628211ULL; } free(cfg); }
    }
    const char *key=g_target_name?g_target_name:(g_static?"host-static":"host");
    char *cdir=sfmt("%s%crt%c%s",cache_root(),PATHSEP,PATHSEP,key);
    mkdir_p(cdir);
    char *obj=sfmt("%s%crt_%08lx_t%dv%dd%d.o",cdir,PATHSEP,
                   (unsigned long)(h & 0xffffffffUL),use_tls?1:0,no_verify?1:0,dbg?1:0);
    if(file_exists(obj)) return obj;
    Buf c={0}; char ccp[1024]; cc_prefix(ccp,sizeof ccp);
    buf_addf(&c,"%s -std=c99 %s -I\"%s\"",ccp, dbg?"-O0 -g -DCMM_DEBUG":"-O2 -ffunction-sections -fdata-sections", tmpdir);
    if(use_tls) buf_addf(&c," -DCMM_HAVE_TLS -I\"%s/mbedtls/include\" -I\"%s\" \"-DMBEDTLS_CONFIG_FILE=<cmm_mbedtls_config.h>\"",g_tls_dir,g_tls_dir);
    if(use_tls && no_verify) buf_add(&c," -DCMM_TLS_NO_VERIFY");
    buf_addf(&c," -c \"%s\" -o \"%s\"",runtime_c,obj);
    int rc=run_cmd(c.p,verbose?0:1); free(c.p);
    if(rc!=0){ remove(obj); return NULL; }           /* fall back to source compile */
    return file_exists(obj)?obj:NULL;
}
...
static void compile_to_exe(const char *c_path,const char *out_path,int verbose,int tls,int uses_http){
    setup_target();
    resolve_toolchain();
    if(!g_cc) die("zig toolchain not found.\n"
                  "  cmm compiles through zig (one C toolchain for every target). Options:\n"
                  "    - install zig:  pip install ziglang     (Windows, macOS, Linux)\n"
                  "    - or put zig from ziglang.org on PATH, or set CMMC_ZIG=<path>\n"
                  "    - advanced: set CMMC_CC=<gcc-like cc>, or pass --cc <path>");
    ...
    if(want_tls){
        if(ensure_tls_lib(verbose)) use_tls=1;
            ...
            die("TLS requested (--tls) but the bundled TLS library was not found.\n"
                ...
                "  a 'third_party/mbedtls' directory next to cmm (or set CMMC_TLS_DIR),\n"
                "  and a zig backend (pip install ziglang). See docs/networking.");
    }

    /* cached, precompiled runtime object (falls back to source if unavailable) */
    char *rt_obj=ensure_runtime_obj(runtime_c,tmpdir,use_tls,g_no_verify,g_debug,verbose);
    const char *rt_in = rt_obj ? rt_obj : runtime_c;
    ...
    {                                      /* zig cc (gcc-like) */
        ...
        buf_addf(&cmd," \"%s\" \"%s\" -o \"%s\" -lm",c_path,rt_in,out_path);
    ...
    if(verbose||g_debug){
        printf("  cc: %s\n",g_cc);
        printf("  %s\n",cmd.p);
        printf("  TLS: %s\n", use_tls?"enabled (mbedTLS)":"disabled (https:// will return empty)");
    }
...
static char *do_build(const char *entry_path,const char *out_opt,int emit_c,int keep_c,
    ...
    int uses_http = strstr(c_src,"cmm_http_")!=NULL || strstr(c_src,"cmm_mysql_connect_tls")!=NULL;
...
static const char *USAGE =
    "cmm - the C-- (cmm) compiler\n"
    "Usage:\n"
    "    cmm build <file.cmm> [-o OUT] [--emit-c] [--keep-c] [--tls|--no-tls] [-v]\n"
    "    cmm run   <file.cmm> [-v] [-- ARGS...]\n"
    "    cmm emit  <file.cmm>            # write the generated C and stop\n"
    "    cmm check <file.cmm>            # parse + type-check only; reports all errors\n"
...
static int cmd_build(int argc,char **argv,int do_emit){
    ...
    if(!file){ fprintf(stderr,"cmm build: expected exactly one .cmm file\n"); return 2; }
    Vec remote={0};
...
static int cmd_run(int argc,char **argv){
    ...
    if(!file){ fprintf(stderr,"cmm run: expected exactly one .cmm file\n"); return 2; }
    Vec remote={0};
...
static int cmd_check(int argc,char **argv){
    ...
    if(!file){ fprintf(stderr,"cmm check: expected exactly one .cmm file\n"); return 2; }
    Vec remote={0};
    g_collect=1;
